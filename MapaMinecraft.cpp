...
i32 main() {
	...
	/*for (int i = 0; i < transition.size()-2; i++)
	{

		cout << "-------------------" << endl;
		cout << transition[i].x << endl;
		cout << transition[i].y << endl;
		cout << transition[i].z << endl;
	}*/
	srand(time(NULL));
	...
	while (!glfwWindowShouldClose(window)) {
		...
		/*if(PosCuboLuz.x >=20|| PosCuboLuz.z>=20)
		{
			PosCuboLuz.x = 20.0f;
			PosCuboLuz.z = 20.0f;
		}*/
		//model = glm::translate(model, PosCuboLuz);
		...
		{
			PosComida.x = 1 + rand() % (15);
			//PosComida.y = 1 + rand() % (10);
			PosComida.z = 1 + rand() % (15);
		}
...
/*
		for (auto& vec : transition)
		{
			...
			/*for (int i = 0; i < transition.size() - 2; i++)
			{
				if (sqrtl(pow((PosCuboLuz.x - transition[i].x * 2.0f), 2) + pow((PosCuboLuz.y - transition[i].y * 2.0f), 2)) <= 0.1f)
				{
					transition[i].x = 9999;
					transition[i].y = 9999;
					transition[i].z = 9999;
			...
			/*if(sqrtl(pow((PosCuboLuz.x - vec.x*2.0f), 2) + pow((PosCuboLuz.y - vec.y*2.0f), 2)+pow((PosCuboLuz.z - vec.z*2.0f), 2))<0.5f)
			{
				vec.x = 9999;
				vec.y = 9999;
				vec.z = 9999;
